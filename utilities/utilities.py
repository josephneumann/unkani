...
def create_random_user(users=1, gender=None, **kwargs):
        try:
            users = int(users) #TODO handle ValueError for non int parameters
        except ValueError:
            ...
            username_list = []
            email_domains = ['@gmail.com','@icloud.com','@yahoo.com','@microsoft.com','@aol.com','@comcast.com','@mail.com','@inbox.com','@outlook.com']
            ...
            while len(username_list) < users:
                ...
                first_name = get_first_name(gender).upper()
                last_name = get_last_name().upper()
                username = first_name + '.' + last_name + random_number
                rand_domain = email_domains[rand_email_index].upper()
                email = username + rand_domain
                dob = random_dob()
                phone = random_phone()
                user = User(first_name=first_name, last_name=last_name, email=email, username=username, dob=dob, phone=phone)
                if password:
...
def initialize_user_admin():
    ...
    user = User(email=admin_email.upper(), role_id=0, password=admin_password, first_name='Joseph'.upper(), last_name='Neumann'.upper()
                , dob='1990-01-01', phone='262-555-8899', username='jneumann'.upper(), confirmed=True)
    db.session.add(user)
    db.session.commit()
